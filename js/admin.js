...
async function dashboardPage(){
    ...
    try{
        ...
        /* الموظف الذي يجب أن يرفع التقرير:
           نشط + role employee فقط.
           مدير القسم والمدير العام لا يدخلان إطلاقًا في قائمة المتأخرين. */
        const reportingEmployees=employeesData.filter(isReportingEmployee);
        const managers=employeesData.filter(e=>
            normalizeRole(e.role)==="manager" &&
            !["inactive","موقوف","غير نشط","غيرنشط","متوقف","مفصول","محذوف"].includes(normalizeText(e.status))
        );

        const reportingIds=new Set(reportingEmployees.map(e=>employeeIdKey(e.id)).filter(Boolean));
        const uploadedIds=new Set(
            weekReports
                .filter(r=>reportingIds.has(employeeIdKey(r.employeeId)))
                .map(r=>employeeIdKey(r.employeeId))
                .filter(Boolean)
        );

        const uploaded=reportingEmployees.filter(e=>uploadedIds.has(employeeIdKey(e.id)));
        const missing=reportingEmployees.filter(e=>!uploadedIds.has(employeeIdKey(e.id)));

        /* تجميع الأقسام من الموظفين أنفسهم، وليس من التقارير.
           بهذا تظهر الأقسام حتى لو لم يرفع أي موظف منها تقريرًا. */
        const departments=new Map();
        reportingEmployees.forEach(e=>{
            const key=normalizeDepartment(e.department)||"__none__";
            if(!departments.has(key)) departments.set(key,{name:String(e.department||"بدون قسم").trim()||"بدون قسم",employees:[]});
            departments.get(key).employees.push(e);
        });

        const deptRows=[...departments.values()].map(dept=>{
            const total=dept.employees.length;
            const up=dept.employees.filter(e=>uploadedIds.has(employeeIdKey(e.id))).length;
            const miss=total-up;
