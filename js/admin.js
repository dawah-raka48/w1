...
async function dashboardPage(){
    ...
    try{
        ...
        const weekReports = currentWeek
            ? reportsData.filter(r => String(r.week) === String(currentWeek))
            : reportsData;
        ...
        // المديرون لا يدخلون ضمن الموظفين المطلوب منهم رفع التقرير.
        ...
        // نحسب الموظف مرة واحدة حتى لو كان هناك أكثر من سجل للتقرير.
        const uploadedEmployeeIds = new Set(
            weekReports.map(r => String(r.employeeId))
        );

        const uploadedEmployees = reportingEmployees.filter(e =>
            uploadedEmployeeIds.has(String(e.id))
        );
        ...
        const departments = [...new Set(
            reportingEmployees.map(e => e.department).filter(Boolean)
        )];

        const deptRows = departments.map(dept => {
            const deptEmployees = reportingEmployees.filter(e =>
                e.department === dept
            );
